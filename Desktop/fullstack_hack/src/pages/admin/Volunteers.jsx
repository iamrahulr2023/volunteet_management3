...
import { mockWorkBalanceData } from '../../data/mockData';
        ...
        {filtered.map(vol => (
          <div key={vol.id} className="card">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={statusStyles[vol.status]?.badge}>{statusStyles[vol.status]?.text}</span>
            </div>

            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2 text-gray-500">
                <MapPin className="w-4 h-4 text-primary-400" /> {vol.location}
              </div>
              <div className="flex items-center gap-2 text-gray-500">
                <Clock className="w-4 h-4 text-primary-400" /> {vol.totalHours} hours • {vol.availability}
              </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              <span className={`badge text-xs border ${workloadColors[vol.workload]}`}>
                {vol.workload === 'low' ? '🟢' : vol.workload === 'medium' ? '🟡' : '🔴'} {vol.workload} workload
              </span>
              <button
